...
class Siamese(nn.Module):
    def __init__(self, hidden_dim, num_classes, tokenizer_path='neuralmind/bert-large-portuguese-cased', model_path='neuralmind/bert-large-portuguese-cased'):
    ...
    def embed(self, q):
        input_ids = self.tokenizer.encode(q, return_tensors='pt', truncation=True, max_length=256).to(self.device)
    ...
    def forward(self, q1, q2):
        token_ids = self.tokenizer(q1, return_tensors='pt', padding=True, truncation=True, max_length=256)
        input_ids = token_ids['input_ids'].to(self.device)
        vector = self.bert(input_ids)
        q1_emb = vector['last_hidden_state'][:, 0, :]
        
        token_ids = self.tokenizer(q2, return_tensors='pt', padding=True, truncation=True, max_length=256)
        input_ids = token_ids['input_ids'].to(self.device)
        vector = self.bert(input_ids)
        q2_emb = vector['last_hidden_state'][:, 0, :]

        X = torch.cat([q1_emb, q2_emb], 1)
        probs = self.softmax(self.linear(X))
        return q1_emb, q2_emb, probs
...
def prepare_data(trainset, candidate_format='question'):
    ...
    for qa in traindata:
        ...
        for i, row in enumerate(qa['candidates']):
            ...
            label = 0 if row['label'].lower().strip() == 'i' else 1
...
def test(model, testdata, candidate_format='question'):
    ...
    for query_idx, qa in enumerate(testdata):
        ...
        q1_emb, q2_emb, output = model(len(batch_candidates) * [q1], batch_candidates)
        rank = list(torch.argmax(output, 1))
        
        y_pred.extend(rank)
        y_real.extend(batch_y)

        scores = model.distance(q1_emb[0], q2_emb)
        results[qid] = [("false", float(score)) if batch_y[i] == 0 else ("true", float(score)) for i, score in enumerate(scores)]

    y_pred = [int(w) for w in y_pred]
    y_real = [int(w) for w in y_real]
    
    f1 = f1_score(y_real, y_pred)
    acc = accuracy_score(y_real, y_pred), set(y_pred)
    return results, f1, acc
...
if __name__ == "__main__":
    ...
    for candidate_format in ['question', 'answer', 'question+answer']:
        ...
        for fold in annotatedcv:
            ...
            model = Siamese(hidden_dim, num_classes, tokenizer_path, model_path)
            model.to(device)
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                foldresults, f1, acc = test(model, testdata, candidate_format)
